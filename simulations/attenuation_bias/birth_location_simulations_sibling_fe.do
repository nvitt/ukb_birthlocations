...
keep family_id sibling_id birth_date age_gap birth_easting birth_northing
...
program define simulation, rclass
	...
	cap drop loc_error both_sib which_sib x y x_obs diff
	...
	if strpos("`var'","_sibloc") == 0 & strpos("`var'","_midloc") == 0 {
		...
		forvalues i=0(2)10 {
			...
			simulate2 varname=r(varname) b_unbiased=r(b_unbiased) ///
				b_biased=r(b_biased) bias=(r(b_biased)-r(b_unbiased)) diff_loc_share=r(diff_loc_share) ///
...
save "${OUTPUT}/output/simulations/birthloc_simulations_sibling_error_sibling_fe.dta", replace


*** Summary of the simulation output:
tabstat b_unbiased b_biased bias diff_loc_share if temporal_variance_percentage==0, by(rho) stats(mean)
tabstat b_unbiased b_biased bias diff_loc_share if temporal_variance_percentage==20, by(rho) stats(mean)
tabstat b_unbiased b_biased bias diff_loc_share if temporal_variance_percentage==40, by(rho) stats(mean)
tabstat b_unbiased b_biased bias diff_loc_share if temporal_variance_percentage==60, by(rho) stats(mean)
tabstat b_unbiased b_biased bias diff_loc_share if temporal_variance_percentage==80, by(rho) stats(mean)
tabstat b_unbiased b_biased bias diff_loc_share if temporal_variance_percentage==100, by(rho) stats(mean)
...
// Latex export:
esttab bias_by_rho_* using "${OUTPUT}/output/tablefragments/bias_birthloc_simulations_sibling_fe_v3.tex", ///
			cells("mean(fmt(2))") nonumber ///
